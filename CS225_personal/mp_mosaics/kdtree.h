...
private:
   ...
   /**
     ...
     */
   void deleteNode(KDTreeNode *subRoot);
   KDTreeNode *createSubtree(vector<Point<Dim>> &points, int a, int b, int curDim);
   int quickSelectN(vector<Point<Dim>> &points, int a, int b, int curDim, int n);
   int partition(vector<Point<Dim>> &points, int a, int b, int curDim);
   Point<Dim> findNearestNeighbor(KDTreeNode *subroot, const Point<Dim> &query, int dimension) const;
   double distanceSquared(const Point<Dim> &first, const Point<Dim> &second) const;
};
