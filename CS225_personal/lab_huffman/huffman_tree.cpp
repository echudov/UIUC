                            ...
                            queue<TreeNode *> &mergeQueue)
{
    ...
    else if (mergeQueue.size() == 0 || (singleQueue.front())->freq < (mergeQueue.front())->freq)
    {
...
void HuffmanTree::buildTree(const vector<Frequency> &frequencies)
{
    ...
    {
        HuffmanTree::TreeNode *smallest = HuffmanTree::removeSmallest(singleQueue, mergeQueue);
        HuffmanTree::TreeNode *secondSmallest = HuffmanTree::removeSmallest(singleQueue, mergeQueue);
        HuffmanTree::TreeNode *combined = new HuffmanTree::TreeNode(smallest->freq.getFrequency() + secondSmallest->freq.getFrequency());
...
HuffmanTree::TreeNode *HuffmanTree::readTree(BinaryFileReader &bfile)
{
    ...
    while (bfile.hasBits())
    {
        if (bfile.getNextBit())
        {
            HuffmanTree::TreeNode *leaf = new TreeNode(Frequency(bfile.getNextByte(), 0));
            return leaf;
        }
        else
        {
            HuffmanTree::TreeNode *node = new HuffmanTree::TreeNode(0);
            node->left = HuffmanTree::readTree(bfile);
            node->right = HuffmanTree::readTree(bfile);
            return node;
        }
    }
    return NULL;
